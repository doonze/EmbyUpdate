...
def config_setup(version):
    ...
    if serverinfo.enablecheck:
        if serverinfo.version is None:
            print()
            print(f"{c.fg.lt_red}I didn't find a running Emby instance on this server.{c.end}")
            ...
            print(f"{c.end}")
            print(f"[{c.fg.pink}1{c.end}] {c.fg.lt_blue}You can skip this check for now{c.end}")
            print(f"[{c.fg.pink}2{c.end}] {c.fg.lt_blue}Permanently disable this check{c.end}")
            print(f"[{c.fg.pink}3{c.end}] {c.fg.lt_blue}Update the server address{c.end}")
            print(f"[{c.fg.pink}4{c.end}] {c.fg.lt_blue}Cancel this run (maybe to start the server?){c.end}")
            print()
